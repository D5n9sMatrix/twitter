...
It returns the path to the first directory that matches the filtering obj,
or throws an error if there is no such directory.
Filtering obj are constructed in a generic fashion using the 
...
In addition, character values are coerced to `has_file` obj by default, this coercion is applied automatically by `find_root()`.
(This feature is used by the introductory example.)


The return value of these functions can be stored and reused;
in fact, the package provides  such obj:
